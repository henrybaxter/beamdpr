...
class BinTestCase(TestCase):
    ...
    def test_combine_one(self):
        input_files = ['test_data/first.egsphsp']
        output_file = 'test_data/test_combined_output.egsphsp'
        result = run(['cargo', 'run', '--', 'combine'] + input_files + ['-o', output_file], stderr=PIPE, stdout=PIPE)
        self.assertEqual(result.returncode, 0)
        self.assertTrue(filecmp.cmp(output_file, 'test_data/first.egsphsp', shallow=False))

    def test_combine_two(self):
        input_files = ['test_data/first.egsphsp', 'test_data/second.egsphsp']
        output_file = 'test_data/test_combined_output.egsphsp'
        result = run(['cargo', 'run', '--', 'combine'] + input_files + ['-o', output_file], stderr=PIPE, stdout=PIPE)
        self.assertEqual(result.returncode, 0)
        self.assertTrue(filecmp.cmp(output_file, 'test_data/combined.egsphsp', shallow=False))
        os.remove(output_file)

    def test_translate(self):
        input_file = 'test_data/first.egsphsp'
        output_file = 'test_data/test_translate.egsphsp'
    ...
    def test_rotate(self):
        input_file = 'test_data/first.egsphsp'
        output_file = 'test_data/test_rotate.egsphsp'
    ...
    def test_reflect(self):
        input_file = 'test_data/first.egsphsp'
        output_file = 'test_data/test_reflect.egsphsp'
